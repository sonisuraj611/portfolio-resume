...
const Navbar = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden fixed inset-0 bg-navy-light bg-opacity-95 z-40 transition-all duration-300 flex flex-col items-center justify-center ${
        ...
        <nav className="flex flex-col items-center space-y-6">
          ...
          <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" onClick={() => setIsOpen(false)}>
            <Button variant="outline" className="border-teal text-teal hover:bg-teal/10 mt-4">
              Resume
            </Button>
          </a>
        </nav>
